...
def _ensure_min_months(
    df: pd.DataFrame, cutoff_month: pd.Timestamp, min_months: int
) -> pd.DataFrame:
...
def _build_future_frame(
    df: pd.DataFrame, predict_month: pd.Timestamp, ok_pairs: pd.DataFrame
) -> pd.DataFrame:
...
def _preprocess_for_model(full_df: pd.DataFrame, predict_month: pd.Timestamp):
    ...
    meta = (
        test[meta_cols]
        if all([m in test.columns for m in meta_cols])
        else df[df["date"] == predict_month][meta_cols]
    )
...
def _train_predict(X_train, y_train, X_test):
    ...
    for tr_idx, va_idx in tscv.split(X_train):
        ...
        val_scores.append(
            (
                r2_score(y_train.iloc[va_idx], val_pred),
    ...
    y_pred = (
        ...
        if preds
        else lgb.LGBMRegressor().fit(X_train, y_train).predict(X_test)
    )
...
def predict_order(file_storage):
    if file_storage is None:
        return (
            jsonify({"success": False, "data": None, "error": "CSV file is missing"}),
            400,
        )
            ...
            jsonify(
                {
                    "success": False,
                    "data": None,
                    "error": f"failed to read uploaded csv: {e}",
                }
            ),
            400,
        )
            ...
            jsonify(
                {
                    "success": False,
                    "data": None,
                    "error": f"missing columns in upload: {sorted(list(missing))}",
                }
            ),
            400,
        )
            ...
            jsonify(
                {
                    "success": False,
                    "data": None,
                    "error": f"failed to concat/dedupe: {e}",
                }
            ),
            500,
        )
    if merged.empty:
        return (
            jsonify({"success": False, "data": None, "error": "no data after merge"}),
            400,
        )
    ...
    if pd.isna(upload_month):
        return (
            jsonify({"success": False, "data": None, "error": "invalid upload month"}),
            400,
        )
    predict_month = (upload_month + pd.DateOffset(months=1)).replace(day=1)

    min_months = _get_min_months()
    ok_pairs = _ensure_min_months(
        merged, cutoff_month=upload_month, min_months=min_months
    )
    if ok_pairs.empty:
        return (
            jsonify(
                {
                    "success": False,
                    "data": None,
                    "error": f"At least {min_months} months of history required for each (store_id, product_code).",
                }
            ),
            400,
        )
    ...
    try:
        X_train, y_train, X_test, meta = _preprocess_for_model(
            full_df, predict_month=predict_month
        )
                ...
                jsonify(
                    {
                        "success": False,
                        "data": None,
                        "error": "Not enough rows to train or predict",
                    }
                ),
                400,
            )
        ...
        return (
            jsonify({"success": False, "data": None, "error": f"model failure: {e}"}),
            500,
        )
...
#     print("✅ 발주 예측 서버 실행 중... http://localhost:5000")
