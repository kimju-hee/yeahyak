...
def summarize_pdf(text):
    ...
    # 프롬프트 구성
    user_prompt = f"""
    다음은 의약품 설명서입니다. 아래의 설명에 따라 1000자 이내 요약문을 작성하세요.

    [HTML 출력 규칙]
    - 마크다운/코드펜스 금지: 백틱(```) 및 ```html 금지
    - DOCTYPE, <html>, <head>, <body> 없이 '본문만' 출력하세요.
    - 허용 태그: <h1>, <h2>, <h3>, <p>, <ul>, <ol>, <li>, <strong>, <em>, <br>
    - style/script/onclick 등 속성 사용 금지

    [작성 지침]
    - 전체 길이: 500자 이내로 간결하고 핵심적인 내용
    - 출력은 반드시 <h2> 또는 <h3>으로 시작하세요.
    - 다음의 섹션을 반드시 포함하세요.: 성분, 효능, 사용법, 주의사항, 보관법
