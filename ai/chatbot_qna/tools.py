...
def get_drug_general_info(item_name: str) -> str:
    ...
    try:
        ...
        if not items:
            return f"API 조회 결과, '{item_name}'에 대한 정보를 찾을 수 없었습니다."
        return str(items[0])
...
def get_ingredient_contraindication_info(ingredient_name: str) -> str:
    ...
    base_url = (
        "http://apis.data.go.kr/1471000/DURIrdntInfoService03/getUsjntTabooInfoList02"
    )
    params = {
        "serviceKey": SERVICE_KEY,
...
def get_ingredient_general_info(ingredient_name: str) -> str:
    ...
    try:
        ...
        for item in items:
            info = [
                f"일반명: {item.findtext('gnlNm')}",
                f"분류명: {item.findtext('divNm')}",
            ]
